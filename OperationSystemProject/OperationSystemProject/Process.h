...
public:
    ...
    void set_waiting_time(float waiting_time){
        this-> waiting_time=waiting_time;
    }
    void set_response_time(float response_time){
        this-> response_time=response_time;
    }
    void set_turnaround_time(float turnaround_time){
        this-> turnaround_time=turnaround_time;
    }
    void set_remaning_time(float remaining_time){
...
Gantt info;
void RR(float *curr_time,float CPU_time){
    int Qtime=8;
    while(Qrr8.size()>0 && *curr_time<CPU_time){
        Process p=Qrr8.front();
        if(p.get_burst_time()+ *curr_time>CPU_time){//*check
            if(p.get_response_time()<0){
                p.set_response_time(*curr_time-p.get_arrival_time());
            }
        ...
        if(p.get_burst_time()<=Qtime){
            ...
            *curr_time+=Qtime;
            p.set_turnaround_time(*curr_time-p.get_arrival_time());
            p.set_remaning_time(p.get_remaning_time()-Qtime);
            Qrr16.push(p);
            Qrr8.pop();
...
void RR16(float *curr_time,float CPU_time){
    int Qtime=16;
...
void FCFS(float *curr_time, float CPU_time){
    Process p = Qfcfs.front();
    if(p.get_remaning_time()+ *curr_time>CPU_time && !Qrr8.empty()){
        p.set_remaning_time(p.get_remaning_time()-(CPU_time-*curr_time));
        p.set_waiting_time((*curr_time-p.get_turnaround_time())+p.get_waiting_time());
        *curr_time+=(CPU_time-*curr_time);
        p.set_turnaround_time(*curr_time-p.get_arrival_time());
...
void Execute(){
    ...
    while(curr_time<total_time){
        if(curr_time<floor(Q1_CPU_time)&&!Qrr8.empty()){
            RR(&curr_time,Q1_CPU_time);
            cout<<curr_time<<endl;
        }
        else if(curr_time<floor(Q2_CPU_time)&&!Qrr16.empty()){
            RR16(&curr_time,Q2_CPU_time);
            cout<<curr_time<<endl;
        }
        else {
            FCFS(&curr_time,Q3_CPU_time);
        }
